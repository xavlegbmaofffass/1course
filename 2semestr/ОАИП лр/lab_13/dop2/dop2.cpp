...
struct Object {
    ...
    void resize();                 // Функция для изменения размера таблицы
};

static void* DEL = (void*)HASHDEL;
Object create(int size, string(*getkey)(void*));
#undef HASHDEL
...
Object::Object(int size, string(*getkey)(void*)) {
    N = 0;
    this->size = size;
    this->getKey = getkey;
...
void Object::resize() {
    ...
    delete[] oldData;
    collisions.clear();
    cout << "Таблица увеличена до размера: " << newSize << endl;
}
...
int main() {
    ...
    for (;;) {
        ...
        case 1:
            H.scan(AAA_print);
            cout << "Текущий размер таблицы: " << H.size << endl;
            cout << "Количество элементов: " << H.N << endl;
            cout << "Заполненность: " << (double)H.N / H.size * 100 << "%" << endl;
            break;
        case 2: {
            AAA* a = new AAA;
            char* str = new char[20];
            cout << "Введите ключ (строка):" << endl;
            string k;
            cin >> k;
            a->key = k;
        ...
        case 5:
            ...
            break;
        case 6: {
            cout << "Измените начальный размер таблицы (16, 32, 64, 128): ";
            int newSize;
            cin >> newSize;
            if (newSize == 16 || newSize == 32 || newSize == 64 || newSize == 128) {
                siz = newSize;
                cout << "Создание новой таблицы размером " << siz << endl;
